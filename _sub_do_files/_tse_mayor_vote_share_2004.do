...
gen keepar = regexm(DESCRICAO_ELEICAO, "ELEICOES 2004") // Keep obs of regular elections
drop if keepar ~= 1
drop keepar	
...
append using "$tmp/tse_mayor_winners_2000.dta"
...
by cod_tse, sort: gen iten01 = voto if year_of_election == 2004 & year_of_election[_n-1] == 2000 & numero_urna[_n] == numero_urna[_n-1] 
by cod_tse, sort: egen iten02 = mean(iten01) //vote for a party trying reelection

gsort  cod_tse  year_of_election name_of_winner 
by cod_tse, sort: gen iten03 = voto if year_of_election == 2004 & year_of_election[_n-1] == 2000 & name_of_winner[_n] == name_of_winner[_n-1] 
...
save "$tmp/tse_mayor_vote_share_2004.dta", replace
