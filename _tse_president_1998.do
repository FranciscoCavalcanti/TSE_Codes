...
* generating a variable the depicts the vote share of party of the incumbent president
	* first round
by CODIGO_MUNICIPIO, sort: egen leao01 =  total(TOTAL_VOTOS) if NUM_TURNO ==1 & DESCRICAO_CARGO =="PRESIDENTE" // total vote for president
by CODIGO_MUNICIPIO, sort: egen total_vote_by_city_1round =  mean(leao01) 
by CODIGO_MUNICIPIO, sort: egen iten01 =  total(TOTAL_VOTOS) if NUMERO_PARTIDO==45 & NUM_TURNO ==1 & DESCRICAO_CARGO =="PRESIDENTE" // total vote for incument party 
...
collapse (mean) vote_share_pres_1round, by(CODIGO_MUNICIPIO)
...
gen year_of_election = 1998

* keep variables

keep cod_tse year_of_election vote_share_pres_1round
